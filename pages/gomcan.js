  ...
  return (
    <div style={{ ...card, padding: 16, marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
        <h3 style={{ fontWeight: 800, margin: 0 }}>🏠 Gia dụng + Thực phẩm chức năng ({list.length})</h3>
        <ViewModeToggle mode={viewMode} setMode={setViewMode} T={T} />
      </div>
      {list.length === 0 && <div style={{ color: THEME.subtext, fontSize: 16, marginTop: 8, marginBottom: 8 }}>Chưa có sản phẩm nào</div>}

      <div style={{ display: "grid", gridTemplateColumns: gridColumnsFor(viewMode), gap: 10, marginTop: 12, marginBottom: 16 }}>
        {list.map((it, i) => (
          <GiadungCard
      ...
      <div className="hnCard" onClick={onOpen} style={{ ...card, minWidth: 0, maxWidth: "100%", cursor: "pointer", display: "flex", gap: 10, padding: 10, alignItems: "center" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 2 }}>
            <span style={{ ...chip, fontSize: 11, padding: "1px 8px" }}>{isReady ? "Hàng sẵn" : "Hàng order"}</span>
