...
# node scripts/get-token.js khi hết hạn token
...
3. `APIs & Services` → `OAuth consent screen` → chọn _External_ (hoặc Internal nếu workspace), điền tối thiểu app name, email.
4. `Credentials` → `Create Credentials` → **OAuth client ID** → _Desktop app_.
