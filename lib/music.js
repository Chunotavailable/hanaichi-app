...
// Chọn ngẫu nhiên 1 bài, tránh lặp lại đúng bài vừa nghe (nếu có từ 2 bài trở lên).
function pickRandomTrack(excludeIdx, total) {
  if (total <= 1) return 0;
  let next = Math.floor(Math.random() * total);
  while (next === excludeIdx) next = Math.floor(Math.random() * total);
  return next;
}
...
export function BackgroundMusic() {
  ...
  // Phát ngẫu nhiên thay vì theo thứ tự danh sách: giữ 1 "lịch sử" các bài đã
  // chọn để nút ⏮ quay lại đúng bài trước đó, còn ⏭/hết bài thì random bài mới.
  const [history, setHistory] = useState(() => [Math.floor(Math.random() * TRACKS.length)]);
  ...
  const trackIdx = history[histPos];
  ...
  // Bài kết thúc -> tự chuyển sang 1 bài NGẪU NHIÊN khác và phát luôn (đây là
  // tiếp nối của phiên phát đã được người dùng cho phép trước đó, không phải
  // phát mới).
  function handleEnded() {
    const next = pickRandomTrack(trackIdx, allTracks.length);
    setHistory((h) => [...h.slice(0, histPos + 1), next]);
    setHistPos((p) => p + 1);
    const a = audioRef.current;
    if (a) {
  ...
  // Bài tiếp theo (⏭): chọn ngẫu nhiên 1 bài mới, khác bài đang phát.
  // Bài trước (⏮): quay lại đúng bài đã phát trước đó (nếu có), không random.
  // Thao tác trực tiếp lên thẻ audio ngay trong onClick để iOS chấp nhận.
  function switchTrack(dir) {
    let next;
    let newHistPos;
    if (dir > 0) {
      next = pickRandomTrack(trackIdx, allTracks.length);
      const newHistory = [...history.slice(0, histPos + 1), next];
      setHistory(newHistory);
      newHistPos = newHistory.length - 1;
      setHistPos(newHistPos);
    } else {
      if (histPos === 0) return; // đã là bài đầu tiên trong lịch sử, không có bài trước
      newHistPos = histPos - 1;
      next = history[newHistPos];
      setHistPos(newHistPos);
    }
    const a = audioRef.current;
    if (!a) return;
