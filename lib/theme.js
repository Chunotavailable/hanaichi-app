...
export function ConfirmDialog({ open, message, onCancel, onConfirm, confirmLabel = "Xoá", cancelLabel = "Huỷ" }) {
  ...
  return (
    <div
      onClick={onCancel}
      style={{ position: "fixed", inset: 0, background: "rgba(60,20,25,0.45)", zIndex: 200, display: "grid", placeItems: "center", padding: 16 }}
    >
      <div
      ...
      >
        ...
        <div style={{ color: THEME.brand, fontWeight: 800, fontSize: 16, lineHeight: 1.5, marginBottom: 18 }}>{message}</div>
        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <button
            onClick={onCancel}
            style={{ background: THEME.surface, color: THEME.text, border: `1px solid ${THEME.line}`, borderRadius: 12, padding: "8px 18px", fontWeight: 600, cursor: "pointer", fontSize: 14 }}
          >
            {cancelLabel}
          </button>
          <button
            onClick={onConfirm}
            style={{ background: THEME.brand, color: "#fff", border: "none", borderRadius: 12, padding: "8px 18px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}
          >
            {confirmLabel}
          </button>
        </div>
